...
class SlidingWindowCache(nn.Module):
    ...
    3. 超出窗口长度的最旧 Token 被物理丢弃，切断其梯度以节省显存

    TBPTT 语义（重要）:
        只有**历史**部分被 detach，**本 chunk** 必须保留在计算图中。
        否则本 chunk 的 K/V 投影层拿不到任何梯度。
        返回给调用方的是"历史(detached) + 本 chunk(带图)"的拼接结果；
        写入自身缓存的是整段 detach 后的副本（供下一次调用当历史用）。
    """
    ...
    def update(self,
        ...
        Returns:
            ...
            [历史缓存(detached) + 当前输入(保留计算图)]，
            总长度不超过 self.window_size
        """
        # 1. 如果缓存为空，当前输入就是唯一的上下文。
        #    注意: 这里必须返回**未 detach** 的 k_s/k_t/v/coords，
        #    只把 detach 后的副本写入缓存。若直接 return self.cache_*，
        #    本 chunk 的 K_s/K_t/V 会脱离计算图，
        #    导致 W_k_s / W_k_t / W_v 永远拿不到梯度。
        ...
        # 2. 拼接: [历史缓存(detached)] + [当前输入(带图)]
        #    历史部分已 detach，因此梯度不会回传到更早的 chunk（TBPTT 策略）。
        ...
        # 3. 如果总长度超过窗口上限，丢弃最旧的部分。
        #    保留末尾 self.window_size 个 Token，因此当前 chunk 始终被保留。
        if full_k_s.size(2) > self.window_size:
        ...
        # 4. 更新缓存（保存 detach 后的副本，阻断跨 chunk 的历史反向传播）
        ...
        # 5. 返回给调用方的是带图的版本（历史部分本就是 detach 的，不影响 TBPTT）
