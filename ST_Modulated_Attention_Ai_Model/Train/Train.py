...
# ============================================================================
# 4. 拼接所有数据集（简单拼接）
#    注意: 验证集 lccc_valid_dataset 绝不能进入训练集，否则 val loss 失去意义。
#    （此前 all_datasets 里误含 lccc_valid_datasets，导致同一份数据既训练又验证）
# ============================================================================
...
print(f"验证集样本数:   {len(lccc_valid_dataset)}")
...
def collate_with_global_tokenizer(batch, tokenizer=tokenizer):
    ...
    # 确保 coords_raw 是 Tensor（HF 记录里的 coords_raw 是二维 list）
    coords_tensors = []
    for c in coords_raw:
    ...
    # attention_mask 必须是整型: PhysicsRegularizationLoss 内部会做
    # `valid_mask & diag_mask`，float mask 会抛
    # RuntimeError: "bitwise_and_cpu" not implemented for 'Float'
    masks_tensors = [m.to(torch.long) if isinstance(m, torch.Tensor)
                     else torch.tensor(m, dtype=torch.long) for m in attention_masks]

    padded_input_ids = pad_sequence(input_ids, batch_first=True, padding_value=tokenizer.pad_token_id)
    padded_masks = pad_sequence(masks_tensors, batch_first=True, padding_value=0)
    padded_coords = pad_sequence(coords_tensors, batch_first=True, padding_value=0.0)
...
def train():
    ...
    for epoch in range(100):
        ...
        for batch in progress_bar:
            ...
            if scaler:
                with autocast():
                    logits, _, coords_norm = model(input_ids, coords_raw, attention_mask)
                    ce_loss = compute_loss(logits, input_ids, attention_mask)
                    # 物理损失必须作用在模型实际使用的坐标上（coords_norm 含
                    # 可学习归一化层，保留计算图）；用 coords_raw 会因它是数据集
                    # 常量而完全脱离计算图，对模型零梯度贡献。
                    phys_loss, phys_dict = phys_loss_fn(coords_norm, mask=attention_mask)
...
def evaluate(max_batches=2000):  # 限制验证 batch 数，避免过慢
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(val_progress):
            ...
            logits, _, coords_norm = model(input_ids, coords_raw, attention_mask)
            ce_loss = compute_loss(logits, input_ids, attention_mask)
            phys_loss, _ = phys_loss_fn(coords_norm, mask=attention_mask)
